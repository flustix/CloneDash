...
public class MultiplayerClient : IMultiplayerClient
{
    ...
    private void UpdateCharacter(ICharacterDescriptor? desc)
        => _connection.Server.UpdateCharacter(desc?.GetUniqueID() ?? "");
    ...
    Task IMultiplayerClient.LoadMap(string id, int diff)
    {
        MainThread.RunASAP(() =>
        {
            var song = MuseDashCompatibility.Songs.FirstOrDefault(x => x.JsonInfo.UID == id);
            if (song is null)
            {
            ...
            DashGameLevel.LoadLevel(song, diff, false);
        });
